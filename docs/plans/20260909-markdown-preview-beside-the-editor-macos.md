...
- [x] Write it in the established style, reusing `LSPSourceGatingTests.strippingCommentsAndStringLiterals(_:)`, with the feature's file set enumerated explicitly and the shared hosts (`ContentView.swift`, `PisakaApp.swift`, `CodeEditorView.swift`, `SyntaxTheme.swift`, `FoldCommands.swift`) named as such. (`MarkdownPreviewSourceGatingTests`, 12 tests; the file set and the five shared hosts are static tables the suite re-checks exist before anything else.)
- [x] Pin, by set equality where the rule is a set: `import Markdown` in exactly one app file; the feature's `import WebKit` in exactly one file; every app file of the feature inside `#if os(macOS)` and named by no file under `Sources/Pisaka/iOS/`. (`import Markdown` → `MarkdownParser.swift` and nowhere in Core; the feature's `import WebKit` → `MarkdownPreviewWebView.swift`; all six app files open with `#if os(macOS)` and the iOS layer names none of the feature's twenty spellings.)
- [x] Pin the reader rule: neither `autosave` nor `localChanges` is named anywhere in the feature. (all sixteen files, app and Core, checked for `autosave`/`localChanges`.)
- [x] Pin the one-origin rule: the feature spells `loadHTMLString` **nowhere**, the shell is loaded (`load(URLRequest`) from exactly one app site, and every body update goes through `evaluateJavaScript`. (`loadHTMLString` absent from the feature and from Core; one `.load(URLRequest(` in the feature, in the web view; `evaluateJavaScript` in that file alone and nowhere in Core.)
- [x] Pin the mapping rule: the scheme name, the shell URL, the host and the bundled-file prefix are spelled in exactly one Core file and in no app file; no app file of the feature spells a path-component split, `pathComponents` or `URLComponents` against a preview URL — the inverse is Core's. (the five vocabulary literals — scheme, host, shell path, both prefixes — each in `MarkdownPreviewPage.swift` alone; no app file spells `pathComponents`, `URLComponents` or a path split; `MarkdownPreviewAsset` named by the handler alone, `MarkdownLinkRule` by the web view alone.)
- [x] Pin that `markdownPreviewEnabled` is written from exactly one app site, and the focus helper's single definition with its six call sites, with no file of the feature spelling `firstResponder as? EditorTextView` itself. (one writer, the View menu's toggle; `ContentView` the one named reader; `firstResponder as? EditorTextView` in `EditorCommandTarget.swift` alone, with 5 + 1 call sites and the one passthrough conformer.)
- [x] Pin "no app file of the feature composes HTML" — no `<` tag literal outside Core — using the **literal-keeping** scan (`GitHubSourceGatingTests.strippingComments(_:)`'s form), because the tag *is* a string literal and the shared scanner would delete the very thing the rule checks; record this as the suite's **one stated exception** in its doc comment. (four tag shapes, each impossible in Swift code, checked against the six app files — and each one checked *positively* against Core's own page in `testTheAbsenceRulesArePositivelyControlled`, alongside controls for `loadHTMLString`, both URL splitters and the writer gate, so no absence rule can pass vacuously.)
- [x] Run `swift test`. (5503 Core tests green, 12 of them new; `swiftlint --strict` clean.)
