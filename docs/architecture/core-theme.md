...
speaking the column's name) and `LeetCodeBrowserView.swift` (the browser's query,
through the themed field with the magnifying-glass glyph). The Log bar's doc comment says
...
26. **One field shape and one query toggle.** No gated file spells the
    ...
    LeetCodeBrowserView.swift}` (the commit dialog
    since part five (b), its message box; the two pull-request sheets since
    part five (c); the database grid's cell editor and the problem browser's
    query since part five (d)), plus `ChromeControls.swift`, where the box is composed into the field. The
