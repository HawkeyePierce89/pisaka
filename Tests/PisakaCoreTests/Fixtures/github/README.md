...
| `pr-list-unknown-merge-state.json` | **authored** — one row whose `mergeStateStatus` is `PERHAPS_CLEAN`, a word GitHub declares nowhere, so the fixture is the `GitHubMergeStateStatus` refusal naming `pr list[0].mergeStateStatus`. Deliberately *not* `DRAFT`: that value is deprecated in GitHub's own enum but never removed, so the table carries it and it reads — reaching the draft refusal rather than discarding the whole list over a field that gates one button. |
